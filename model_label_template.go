/*
Benchling API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 2.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package benchlingsdk

import (
	"encoding/json"
)

// LabelTemplate struct for LabelTemplate
type LabelTemplate struct {
	// ID of the label template.
	Id *string `json:"id,omitempty"`
	// Name of the label template.
	Name *string `json:"name,omitempty"`
	// The ZPL template that will be filled in and sent to a printer.
	ZplTemplate *string `json:"zplTemplate,omitempty"`
}

// NewLabelTemplate instantiates a new LabelTemplate object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewLabelTemplate() *LabelTemplate {
	this := LabelTemplate{}
	return &this
}

// NewLabelTemplateWithDefaults instantiates a new LabelTemplate object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewLabelTemplateWithDefaults() *LabelTemplate {
	this := LabelTemplate{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *LabelTemplate) GetId() string {
	if o == nil || isNil(o.Id) {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *LabelTemplate) GetIdOk() (*string, bool) {
	if o == nil || isNil(o.Id) {
    return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *LabelTemplate) HasId() bool {
	if o != nil && !isNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *LabelTemplate) SetId(v string) {
	o.Id = &v
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *LabelTemplate) GetName() string {
	if o == nil || isNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *LabelTemplate) GetNameOk() (*string, bool) {
	if o == nil || isNil(o.Name) {
    return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *LabelTemplate) HasName() bool {
	if o != nil && !isNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *LabelTemplate) SetName(v string) {
	o.Name = &v
}

// GetZplTemplate returns the ZplTemplate field value if set, zero value otherwise.
func (o *LabelTemplate) GetZplTemplate() string {
	if o == nil || isNil(o.ZplTemplate) {
		var ret string
		return ret
	}
	return *o.ZplTemplate
}

// GetZplTemplateOk returns a tuple with the ZplTemplate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *LabelTemplate) GetZplTemplateOk() (*string, bool) {
	if o == nil || isNil(o.ZplTemplate) {
    return nil, false
	}
	return o.ZplTemplate, true
}

// HasZplTemplate returns a boolean if a field has been set.
func (o *LabelTemplate) HasZplTemplate() bool {
	if o != nil && !isNil(o.ZplTemplate) {
		return true
	}

	return false
}

// SetZplTemplate gets a reference to the given string and assigns it to the ZplTemplate field.
func (o *LabelTemplate) SetZplTemplate(v string) {
	o.ZplTemplate = &v
}

func (o LabelTemplate) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !isNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	if !isNil(o.ZplTemplate) {
		toSerialize["zplTemplate"] = o.ZplTemplate
	}
	return json.Marshal(toSerialize)
}

type NullableLabelTemplate struct {
	value *LabelTemplate
	isSet bool
}

func (v NullableLabelTemplate) Get() *LabelTemplate {
	return v.value
}

func (v *NullableLabelTemplate) Set(val *LabelTemplate) {
	v.value = val
	v.isSet = true
}

func (v NullableLabelTemplate) IsSet() bool {
	return v.isSet
}

func (v *NullableLabelTemplate) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableLabelTemplate(val *LabelTemplate) *NullableLabelTemplate {
	return &NullableLabelTemplate{value: val, isSet: true}
}

func (v NullableLabelTemplate) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableLabelTemplate) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


